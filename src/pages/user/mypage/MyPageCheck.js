...
function MyPageCheck() {
  ...
  const getProfile = async() => {
    ...
    await axios.post("/api/myPage", req, {headers: headers})
    .then((response) => {
  ...
  //     await 
  //     axios
  //     .post("/api/myPage",{ password },{ withCredentials: true },{headers: headers});


  //     // 인증 성공 시 MyPage로 데이터 전달
  //     navigate("/mypage", { state: { userData: response.data } });
  //   } catch (error) {
  ...
  return (
    <MyPageContainer>
      ...
      <ButtonContainer>
        ...
        />
        <button type="submit" onClick={getProfile}>확인</button>
          </Article>
      </ButtonContainer>
    </MyPageContainer>
  );
